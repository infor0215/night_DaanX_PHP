...
==========================*/
	$sum=0;//設定記數初始值
	$read_data=0;//設定記數初始值
	$read_SQL=mysql_query("SELECT * FROM night_race_sgin");//尋找資料表
	...
	for ($i=$number; $i <$number+20 ; $i++) { //將＄i設為上次搜索到的最終資料 並以每二十個資料為一循環
		$hear=get_headers("http://night.taivs.tp.edu.tw/news/u_news_v2.asp?id={9514D48C-EAC7-4BD3-A490-EC13A7924A46}&newsid=$i");//取得網站標頭（＄i為上次搜尋到的數字累加）
	...
	if($array_unmber !=""){//當$array_unmber存在時
		...
		mysql_query(" UPDATE night_race_sgin SET id='$last_number'") ;//寫回SQL
		for ($t=0; $t <$last_number ; $t++) { 		
			$ch=curl_init();//curl宣告(第一階段)
			curl_setopt($ch, CURLOPT_URL,"http://night.taivs.tp.edu.tw/news/u_news_v2.asp?id={9514D48C-EAC7-4BD3-A490-EC13A7924A46}&newsid=$array_unmber[$t]");//網頁來源宣告(第一階段)
			...
			$web_main_top2=str_replace('<a href="u_news_v1.asp?id=%7BE8B9CB6F-A0BE-4C02-A5EC-93DE91A606A5%7D&amp;PageNo=&amp;skeyword=">回列表</a>','活動競賽訊息(夜)',$web_main_top);
			$web_main_end=str_replace('活動競賽訊息列表','',$web_main_top2);
			//print_r($web_main_end);
			mysql_query("INSERT INTO night_race (web_id,web) VALUES ('$array_unmber[$t]','$web_main_end') ") ;//寫入資料庫
